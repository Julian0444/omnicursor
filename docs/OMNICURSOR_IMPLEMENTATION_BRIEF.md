...
3. **MCP Tools**
   ...
   - Purpose: structured capabilities that rules and future adapters can call
   - Role: context retrieval, skill loading, compliance checks, future pattern storage
...
- hooks provide automatic behavior without requiring the LLM to decide to call MCP
- MCP provides structured backend capabilities
...
4. If hooks are unavailable, the system still works in degraded MCP mode.
...
- MCP backend lives in [`src/omnicursor/`](/Users/jirustaroure/Desktop/OmniCursor/src/omnicursor)
...
- **If the current hook surface cannot mutate the prompt, keep the same routing logic and fall back to rules + MCP for visible behavior**
...
1. Preserve the current rules and MCP skeleton.
...
- MCP tools that remain usable even without hooks
...
- do not rewrite the project around MCP-only behavior
...
- The approved architecture is Rules + Hooks + MCP
...
3. explain whether the task belongs in Rules, Hooks, or MCP
