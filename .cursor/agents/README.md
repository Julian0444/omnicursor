...
Seventeen files, one per specialized routing profile. Cursor hooks (`on_prompt.py`) and the MCP server both use the same underlying **trigger scoring** (explicit vs context triggers) as defined in these configs.
