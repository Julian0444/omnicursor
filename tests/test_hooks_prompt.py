...
class TestClassifyWithRealAgents:
    def test_debug_prompt(self, agents):
        name, score, _reason = classify_prompt(
    ...
    def test_pr_review_prompt(self, agents):
        name, score, _reason = classify_prompt("Please do a pr review of the latest changes", agents)
        assert score > 0
        assert name == "pr-review"

    def test_summarize_prompt(self, agents):
        name, score, _reason = classify_prompt("Summarize this document for me", agents)
        assert name == "content-summarizer"
        assert score > 0

    def test_commit_prompt(self, agents):
        name, score, _reason = classify_prompt("Help me write a commit message", agents)
        assert name == "commit"
        assert score > 0

    def test_unmatched_prompt_falls_back(self, agents):
        name, score, _reason = classify_prompt("What is the weather today?", agents)
        assert name == "polymorphic-agent"
        assert score == 0.0

    def test_testing_prompt(self, agents):
        name, score, _reason = classify_prompt("Write unit tests for the API module", agents)
        assert name == "testing"
        assert score > 0
...
class TestClassifyEdgeCases:
    def test_empty_prompt_returns_fallback(self, agents):
        name, score, _reason = classify_prompt("", agents)
        assert name == "polymorphic-agent"
        assert score == 0.0

    def test_empty_agents_returns_fallback(self):
        name, score, _reason = classify_prompt("debug this error", [])
        assert name == "polymorphic-agent"
        assert score == 0.0
    ...
    def test_agents_with_missing_fields(self):
        ...
        name, score, _reason = classify_prompt("debug something", bad_agents)
        assert name == "polymorphic-agent"
        assert score == 0.0
    ...
    def test_agents_with_empty_triggers(self):
        ...
        name, score, _reason = classify_prompt("anything", agents)
        assert name == "polymorphic-agent"
        assert score == 0.0
    ...
    def test_case_insensitive_matching(self):
        ...
        name, score, _reason = classify_prompt("I need to debug this", agents)
    ...
    def test_highest_score_wins(self):
        ...
        # agent-a: context trigger "bar baz" → 0.80
        # agent-b: exact explicit trigger "bar" → 0.95
        name, score, _reason = classify_prompt("foo bar baz", agents)
        assert name == "agent-b"
